      ...
      <ElRow :gutter="12">
        <ElCol :span="5">
          <ElFormItem prop="chooseTemple" style="width: 100%">
            <ElSelect
              v-model="form.chooseTemple"
              @change="handlerSelectTemplate"
              placeholder="请选择模板"
            >
              <ElOption
...
import {
  ElMessageBox,
  ElRow, ElCol, ElForm, ElFormItem, ElSelect, ElOption, ElInput,
  ElRadioGroup, ElRadioButton, ElDatePicker, ElCard, ElCascader,
  ElCheckbox, ElTree, ElButton, ElMessage
} from 'element-plus'
// 导入业务相关依赖
...
// 导入子组件（需确保AlertHeader已正确导入）
import AlertHeader from '../components/ReportTemplate/AlertHeader.vue'
...
/** 打开弹窗（原showDia方法，可根据业务调整调用方式） */
const showDia = (row?: Record<string, any>) => {
  Promise.all([
    selectPageList({ pageIndex: 1, pageSize: 999 }),
    areaTreeList()
  ]).then(([tempRes, areaRes]) => {
    tempList.value = tempRes.data.records
    data.value = areaRes.data
    if (!row) {
      // 新增
      resertData()
      checkedAll.value = false
...
const save = async () => {
  ...
  // 校验区域和预览列表
  if (areaIds.value.length === 0) { // 修复原代码赋值=改为判断===
    ElMessage.error("请选择区域")
    return
  }
  ...
  try {
    await nextTick()
    // 生成PDF
    const htmlToPdf = new HtmlToPdf("alertContainer", {
    ...
    // 上传报告并保存数据
    const res = await addReport(myFile, {
      ...form,
