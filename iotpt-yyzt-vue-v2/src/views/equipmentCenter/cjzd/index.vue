    ...
    <div class="index-container">
        ...
        <vab-query-form>
            <vab-query-form-left-panel :span="19" class="left-panel-self">
                <el-form :inline="true" :model="queryForm" @submit.native.prevent ref="queryForm">
                    ...
                    <el-form-item prop="status">
                        <el-select v-model="queryForm.status" placeholder="请选择设备状态" clearable>
                            <el-option
            ...
            <vab-query-form-right-panel :span="5">
                ...
                <el-button icon="el-icon-download" plain @click="downXls"
                           :loading="dowmLoading" :disabled="dowmLoading">
                    {{ dowmLoading ? '导出中' : '导出' }}
                </el-button>
        ...
        <el-table v-loading="listLoading" :data="list" :element-loading-text="elementLoadingText"
                  stripe @selection-change="setSelectRows" height="calc(100vh - 240px)" border>
            ...
            <el-table-column show-overflow-tooltip prop="name" label="设备名称" width="150"
                             align="center"></el-table-column>
            <el-table-column show-overflow-tooltip prop="deviceTypeName" label="类型" width="150"
                             align="center"></el-table-column>
            <el-table-column show-overflow-tooltip prop="subitemName" label="所属分项" width="150"
                             align="center"></el-table-column>
            <el-table-column show-overflow-tooltip prop="areaNames" label="物理位置" align="center" width="250">
            </el-table-column>
            ...
            <el-table-column label="状态" width="100" align="center" prop="status">
                <template slot-scope="{row}">
                    ...
                    <el-link icon="el-icon-info" type="info" v-if="row.status==1"> 未使用</el-link>
                    <el-link icon="el-icon-success" type="success" v-if="row.status==2"> 使用中</el-link>
                    <el-link icon="el-icon-error" type="danger" v-if="row.status==3"> 损坏</el-link>
                    <el-link icon="el-icon-warning" type="warning" v-if="row.status==4"> 维修中</el-link>
                </template>
            </el-table-column>
            <el-table-column label="操作" width="350" align="center">
                <template #default="{ row }">
                    ...
                    <el-button icon="el-icon-view" type="primary" size="mini" @click="handleSignal(row)">信号信息
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    ...
    import {getPageList, saveOrUpdate, onDel, batchUpdate, toExportTemplate} from '@/api/deviceCollect'
                    ...
                    if (logicalAreaId != null && logicalAreaId != "" && logicalAreaId != undefined) {
                        for (var i = 0; i < logicalAreaId.length; i++) {
                            ...
                            if (logicalAreaId[i].level == 3) {
                                this.queryForm.logicalAreaId = logicalAreaId[i].data.value;
                                this.queryForm.logicalType = "3";
                            } else if (logicalAreaId[i].level == 2) {
                                this.queryForm.logicalAreaId = logicalAreaId[i].data.value;
                                this.queryForm.logicalType = "2";
                            } else if (logicalAreaId[i].level == 1) {
                                this.queryForm.logicalAreaId = logicalAreaId[i].data.value;
                                this.queryForm.logicalType = "1";
                            }
                        }
                    } else {
                        this.queryForm.logicalAreaId = '';
                        this.queryForm.logicalType = '';
                    }
...
<style>
    .left-panel-self .left-panel .el-form, .left-panel-self .left-panel .el-form-item__content {
        display: flex;
