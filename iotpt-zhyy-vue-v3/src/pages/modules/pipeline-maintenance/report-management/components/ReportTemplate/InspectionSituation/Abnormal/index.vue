...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        // 表格行类名
        const tableRowClassName = ({ row }: { row: ErrorTask }) => {
        ...
        // 事件处理
        const handleRowClick = (row: ErrorTask) => {
            emit('row-click', row);
        };
        ...
        const handleViewDetail = (row: ErrorTask) => {
            emit('view-detail', row);
        };

        const handleProcess = (row: ErrorTask) => {
            emit('process-task', row);
        };

        const handleViewAttachments = (row: ErrorTask) => {
    ...
    computed: {
        ...
        processingSuggestion() {
            const rate = processingRate;
            if (rate >= 90) return '处理率良好，继续保持！';
            if (rate >= 70) return '处理率正常，建议关注未处理异常';
            if (rate >= 50) return '处理率偏低，请及时处理未处理异常';
            return '处理率过低，请立即处理积压异常！';
        },
        
        // 紧急异常列表
        urgentTasks() {
            const tasks = dataInfo.allErrorTask || [];
            return tasks
