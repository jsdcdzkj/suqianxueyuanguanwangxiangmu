...
export default defineComponent({
    ...
    computed: {
        ...
        formattedData() {
            return {
                maxValue: formatValue(this.dataOptions.maxValue) + '%',
                maxTime: formatDate(this.dataOptions.maxTime),
                date: this.dataOptions.date.map(formatDate),
                value: this.dataOptions.value.map(v => formatValue(v) + '%')
            };
        },
        
        // 负载率趋势分析
        loadTrend() {
            const values = this.dataOptions.value;
        ...
        // 建议措施
        suggestions() {
            const avg = this.dataOptions.value.reduce((a, b) => a + b, 0) / Math.max(1, this.dataOptions.value.length);
