...
import com.jsdc.iotpt.dao.SysBuildDao;
...
public class SysOrgDeptController {
     ...
     * @return
     */
    @PostMapping("/getListDeptList")
    @ApiOperation(value = "部门列表", httpMethod = "POST")
    public ResultInfo getListDeptList(@RequestBody SysOrgDept bean){
        ...
        for (SysOrgDept dept : list) {
            if (org.apache.commons.lang3.StringUtils.isNotBlank(dept.getIds())) {
                List<String> areaIds = Arrays.asList(dept.getIds().split(","));
                List<SysBuildArea> areaList = sysBuildAreaMapper.selectByIds(areaIds);
