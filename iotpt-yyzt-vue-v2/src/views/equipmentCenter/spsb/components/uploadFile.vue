<template>
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormVisible" width="400px" @close="close"
        ...
        >
            ...
            <div class="el-upload__tip" slot="tip">视频设备
...
<script>
import {importUploadVideoData, downloadVideoTemplate} from '@/api/wlpt/deviceCollect'
...
export default {
    ...
    methods: {
        handleExport() {
            // 调用接口获取返回的文件流
            downloadVideoTemplate().then((res) => {
                const blob = new Blob([res], { type: "application/vnd.ms-excel" });
                if ("download" in document.createElement("a")) {
                    // 非IE下载
                    const elink = document.createElement("a");
                    elink.download = '视频设备模板下载.xls';
                    ...
                    // IE10+下载
                    navigator.msSaveBlob(blob, '视频设备模板下载.xls');
                    Message.success('下载成功')
                    setTimeout(() => {
                        Message.success('下载成功')
                    }, 500);
        ...
        async fileChange(file, fileList) {
            ...
            var result = await importUploadVideoData(fd);
