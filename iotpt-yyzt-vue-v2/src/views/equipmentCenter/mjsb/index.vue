<template>
  <div class="index-container">
        ...
        <el-form :inline="true" :model="queryForm" @submit.native.prevent>
          <el-form-item>
            <el-cascader
              ...
              clearable
            ></el-cascader>
            <!-- <el-select v-model="queryForm.areaId" placeholder="选择所属区域" @keyup.enter.native="queryData" clearable>
              <el-option
    ...
    <mjsbView ref="mjsbView" @fetch-data="fetchData"></mjsbView>
  </div>
</template>

<script>
  import { areaTreeList } from '@/api/sysBuildArea'
  import Edit from './components/MjsbEdit'
  import MjsbView from './components/MjsbView'
  import { doorPage, delDoor, saveOrUpdate } from '@/api/mjsb'
  import online from '@/mixins/online'
  export default {
    name: 'UserManagement',
    components: { Edit, MjsbView },
    mixins: [online],
    data() {
      return {
        searchShow: false,
    ...
    methods: {
      ...
      handleView(row) {
        this.$refs['mjsbView'].showEdit(row)
      },
      ...
      async fetchData() {
        ...
        console.log('门禁设备提交操作:', param)
        doorPage(param).then((res) => {
          if (0 == res.code) {
