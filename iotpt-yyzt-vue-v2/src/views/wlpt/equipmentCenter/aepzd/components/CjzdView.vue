    ...
    <el-dialog :close-on-click-modal="false" title="详情" :visible.sync="dialogFormVisible" width="808px" @close="close"
        ...
        <el-descriptions class="descriptions-self" :column="2" border label-class-name="my-label"
            ...
            <el-descriptions-item>
                ...
                <span class="contant" title="正常" v-if="form.onLineStatus == 1">正常</span>
                <span class="contant" title="离线" v-if="form.onLineStatus == 2">离线</span>
                <span class="contant" title="异常" v-if="form.onLineStatus == 3">异常</span>
            </el-descriptions-item>
            <el-descriptions-item>
                <template slot="label">
                    负荷
                </template>
                <span class="contant" :title="form.load">{{ form.load }}</span>
            </el-descriptions-item>
            <el-descriptions-item>
                <template slot="label">
                    所属分项
...
<script>
    import {getEntity} from '@/api/deviceAep'
